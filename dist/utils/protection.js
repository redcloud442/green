...
export const protectionAccountingMerchantAdmin = async (userId, prisma) => {
    try {
        const user = await prisma.user_table.findUnique({
            where: { user_id: userId },
            select: {
        ...
        if (!["ADMIN", "ACCOUNTING", "ACCOUNTING_HEAD", "MERCHANT"].includes(user.alliance_member_table[0].alliance_member_role)) {
            return sendErrorResponse("Invalid Referral Link", 400);
        }
        if (user.alliance_member_table[0].alliance_member_restricted) {
            return sendErrorResponse("Internal Server Error", 500);
