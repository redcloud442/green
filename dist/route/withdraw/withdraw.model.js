...
import { calculateFee, calculateFinalAmount, calculateFinalAmountCash, getPhilippinesTime, } from "../../utils/function.js";
...
export const withdrawCashOutModel = async (params) => {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (existingPackageWithdrawal) {
            throw new Error(`You have already made a cash withdrawal today. Please try again tomorrow.`);
        }
        const amountMatch = await tx.$queryRaw `SELECT
   alliance_combined_earnings,
   alliance_olympus_wallet,
   ...
   FOR UPDATE`;
        ...
        const amountValue = Math.round(Number(amount) * 100) / 100;
        const earningsValue = Math.round(Number(amountMatch[0].alliance_olympus_earnings +
            amountMatch[0].alliance_referral_bounty) * 100) / 100;
        if (amountValue > earningsValue) {
            throw new Error("Insufficient balance.");
        }
        const { olympusEarnings, referralWallet, updatedCombinedWallet } = deductFromWallets(Number(amount), Number(amountMatch[0].alliance_combined_earnings), Number(amountMatch[0].alliance_olympus_earnings), Number(amountMatch[0].alliance_referral_bounty));
        ...
        await prisma.alliance_transaction_table.create({
            ...
            await prisma.alliance_notification_table.create({
                data: {
                    alliance_notification_user_id: teamMemberProfile.alliance_member_id,
                    alliance_notification_message: `Cash Withdrawal request is Ongoing amounting to ₱ ${finalAmount.toLocaleString("en-US", {
                        maximumFractionDigits: 2,
                        minimumFractionDigits: 2,
                    })}. Please wait for approval.`,
                },
...
export const withdrawListPostModel = async (params) => {
    ...
    const orderBy = columnAccessor
        ...
        : Prisma.empty;
    const commonConditions = [
        Prisma.raw(`m.alliance_member_alliance_id = '${teamMemberProfile.alliance_member_alliance_id}'::uuid AND t.alliance_withdrawal_request_withdraw_type != 'CASH'`),
    ];
    ...
    if (teamMemberProfile.alliance_member_role === "ACCOUNTING_HEAD") {
        const totalApprovedWithdrawal = await prisma.alliance_withdrawal_request_table.aggregate({
            where: {
                ...
                alliance_withdrawal_request_date_updated: dateFilter?.start && dateFilter?.end
                    ? {
...
function deductFromWallets(amount, combinedWallet, olympusEarnings, referralWallet) {
