...
export const withdrawPostMiddleware = async (c, next) => {
    ...
    const isAllowed = await redis.rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:withdraw-post`, 50, 60);
    if (!isAllowed) {
...
export const withdrawHistoryPostMiddleware = async (c, next) => {
    ...
    const isAllowed = await redis.rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:withdraw-history-get`, 50, 60);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
...
export const updateWithdrawMiddleware = async (c, next) => {
    ...
    const isAllowed = await redis.rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:update-withdraw`, 100, 60);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
...
export const withdrawListPostMiddleware = async (c, next) => {
    ...
    const isAllowed = await redis.rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:withdraw-list-post`, 100, 60);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
...
export const withdrawHistoryReportPostMiddleware = async (c, next) => {
    ...
    const isAllowed = await redis.rateLimit(`rate-limit:${teamMemberProfile.alliance_member_id}:withdraw-history-report-post`, 100, 60);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
