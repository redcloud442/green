...
export const chatSessionGetMessageMiddleware = async (
  c: Context,
  next: Next
) => {
  const user = c.get("user");
...
export const chatRequestSessionMiddleware = async (c: Context, next: Next) => {
  const user = c.get("user");
...
export const chatSessionGetMessageIdMiddleware = async (
  c: Context,
  next: Next
) => {
  const user = c.get("user");
